...
exports.sendAppointmentConfirmation = onDocumentCreated('/appointments/{documentId}', async (event) => {
  // Get the newly created appointment document
  const appointment = event.data();

  // Get the salon's name
  const salonRef = appointment.salon;
  const salonSnapshot = await salonRef.get();
  const salonName = salonSnapshot.get('name');
  return messaging().sendToTopic('appointments', {
    notification: {
      title: 'New Appointment',
      body: `You have a new appointment at ${salonName}.`,
